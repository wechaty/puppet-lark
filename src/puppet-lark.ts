...
class PuppetLark extends Puppet {
    ...
    async tagContactAdd(tagId: string, contactId: string): Promise<void> {
        ...
        let deps = response.data.data.user_infos.departments
        deps.push(tagId)
    ...
    async contactList(): Promise<string[]> {
        ...
        let authed_employee: string[] = []
        if (response.data.code == 0) {
            let authed_employee_ids = response.data.data.authed_employee_ids
            authed_employee = await this.getEmployeeList(authed_employee_ids)
            return authed_employee
        }
        else {
    ...
    async messageFile(messageId: string): Promise<FileBox> {
        let file_key = this.messageStore[messageId].file_key
        let _token = await this.getTenantAccessToken(this.appId, this.appSecret)
        let response = await axios({
            method: "GET",
            headers: {
        ...
        let file = FileBox.fromBuffer(response.data, '')
        return file
    }
    async messageImage(messageId: string, imageType: ImageType): Promise<FileBox> {
        let image_key = this.messageStore[messageId].image_key
        let _token = await this.getTenantAccessToken(this.appId, this.appSecret)
        let response = await axios({
            method: "GET",
            headers: {
    ...
    async roomList(): Promise<string[]> {
        ...
        let results: string[] = response.data.data.groups
        if (hasmore) {
            while (hasmore) {
                response = await axios({
                    method: "GET",
    ...
    private async getDepartmentList(department_ids: string[]): Promise<string[]> {
        let results = []
        let _token = await this.getTenantAccessToken(this.appId, this.appSecret)
        for (let i = 0; i < department_ids.length; i++) {
            let response = await axios({
                ...
                data: {
                    "open_department_id": department_ids
                }
            })
            if (response.data.code == 0) {
    ...
    private async infomationInit() {
        ...
        if (response.data.code == 0) {
            let authed_employee_ids = response.data.data.authed_open_ids
            this.contacts = await this.getEmployeeList(authed_employee_ids)
            let authed_open_departments = response.data.data.authed_open_departments
            this.departments = await this.getDepartmentList(authed_open_departments)
        }
    }
}
